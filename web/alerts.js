...
(() => {
  ...
  window.renderAlerts = state => {
    ...
    if(loaded&&enabled&&fresh.length&&Notification.permission==='granted'){
      try{const n=new Notification(`发现 ${fresh.length} 条公告提醒`,{body:'打开无终恨意查看官方链接及来源状态',tag:'wuzhong-notices'});n.onclick=()=>{window.focus();tab('alerts');n.close();};}
  ...
  document.querySelector('#read-alerts').onclick=async()=>{
    try{await api('alerts/read','POST',{ids:current.filter(a=>!a.read).map(a=>a.id)});await refresh();}
