...
function syncScope(){
 ...
 const source=$('#source-select').value,ids=scope==='national'?['stats']:scope==='province'?['fujian','guangdong']:state.sources.map(s=>s.id);
 const available=state.sources.filter(s=>ids.includes(s.id)&&(!region.value||s.region===region.value));
...
function applyFilters(f){$('#area-scope').value=f.area_scope||'all';$('#region-select').value=f.region||'';syncScope();for(const key of ['month_from','month_to','year_from','year_to','kind','source_id','notice_scope'])$('#notice-form').elements[key].value=f[key]??(key==='notice_scope'?'all':'');}
$('#area-scope').onchange=syncScope;$('#region-select').onchange=syncScope;
function range(f){return `${f.month_from} 至 ${f.month_to} · ${f.area_scope==='national'?'全国／中央招录':f.region||'全部已接入范围'} · ${f.kind||'全部类型'} · ${f.notice_scope==='public'?'录聘公示及变更':'全部阶段'}${f.source_id?' · '+(state?.sources.find(s=>s.id===f.source_id)?.name||f.source_id):''}${f.year_from?' · 招考'+f.year_from+'—'+f.year_to+'年度':''}`;}
