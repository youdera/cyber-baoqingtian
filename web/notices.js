...
function render(){
 ...
 updateHTML('#source-list',state.sources.filter(s=>!window.sourceCoverageMatches||window.sourceCoverageMatches(s)).map(s=>{const run=state.runs.find(r=>r.sources.some(x=>x.id===s.id));const report=run?.sources.find(r=>r.id===s.id);return `<article class="source-card"><h2>${esc(s.name)}</h2><p>${esc(s.note)}</p><a href="${esc(s.url)}" target="_blank" rel="noreferrer">打开官方栏目 ↗</a><div class="source-bottom">${report?`${esc(statusNames[report.status])} · 读取${report.pages}页 · ${report.found}份公告 · 检查开始 ${esc(stamp(run.started))}`:'本机尚未采集；接入样本核验见项目文档'}${report?.warnings?.map(w=>`<p class="warning">${esc(w)}</p>`).join('')||''}${report?.transport==='p256_compat'?'<p>TLS曲线兼容重试成功，证书验证保持开启。</p>':''}${report?.earliest?`<span>本轮识别日期 ${report.earliest} 至 ${report.latest}（不代表期间无缺口）</span>`:''}</div></article>`;}).join(''));
 ...
 updateHTML('#run-list',state.runs.length?state.runs.map(r=>`<details class="run" data-run="${r.id}" ${opened.has(r.id)?'open':''}><summary>${esc(statusNames[r.status])} · ${stamp(r.started)}</summary><div class="run-body"><p>${esc(range(r.filters))}</p>${r.verification_page_limit?`<p class="warning">本次为接入验收，每栏目最多读取${r.verification_page_limit}页；完整历史未在本轮采集。</p>`:''}<p>公告索引新增 ${r.new} · 元数据更新 ${r.updated}</p>${r.sources.map(s=>`<div class="run-source"><strong>${esc(s.name)}：${esc(statusNames[s.status])}</strong><p>读取${s.pages}页，发现${s.found}份公告。</p>${s.warnings.map(w=>`<p class="warning">${esc(w)}</p>`).join('')}</div>`).join('')}<p>完成时间：${stamp(r.ended)}。栏目读取完成不等于全国或历史完整覆盖。</p></div></details>`).join(''):'<p>首次查询后显示检查记录。</p>');
